...
# revert-rollback-prepare-slice.sh
# Purpose: Single-command revert for rollback_prepare live activation
# Usage:   sudo bash scripts/revert-rollback-prepare-slice.sh <artifacts-dir>
#
...
if [[ $# -lt 1 ]]; then
    ...
    echo "" >&2
    echo "The artifacts-dir is the directory created by activate-rollback-prepare-slice.sh," >&2
    echo "typically: artifacts/phase2/rollback-prepare-live-activation-YYYYMMDD-HHMM" >&2
    exit 1
fi
...
if [[ "$GW_STATUS" != "active" ]]; then
    echo "FAIL: Gateway is not active after revert restart" >&2
fi

if [[ "$BR_STATUS" != "active" ]]; then
    echo "FAIL: Broker is not active after revert restart" >&2
fi
...
# rollback_prepare slice revert result
