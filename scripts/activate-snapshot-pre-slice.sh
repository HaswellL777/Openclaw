...
# activate-snapshot-pre-slice.sh
# Purpose: Single-command operator script for snapshot_pre live activation
# Usage:   sudo bash scripts/activate-snapshot-pre-slice.sh
#
# This script performs the complete snapshot_pre slice activation:
...
# Rollback:   sudo bash scripts/rollback-snapshot-pre-slice.sh <artifacts-dir>
#
# Safety:
...
BACKUP_FILE="${BACKUP_DIR}/index.js.bak-pre-snapshot-pre-slice-${TIMESTAMP}"

# Snapshots
SNAPSHOT_DIR="/.snapshots"
PRE_SNAPSHOT_NAME="root-pre-snapshot-pre-slice-${TIMESTAMP}"
POST_SNAPSHOT_NAME="root-post-snapshot-pre-slice-${TIMESTAMP}"
PRE_SNAPSHOT_PATH="${SNAPSHOT_DIR}/${PRE_SNAPSHOT_NAME}"
POST_SNAPSHOT_PATH="${SNAPSHOT_DIR}/${POST_SNAPSHOT_NAME}"

# Artifacts output
ARTIFACTS_DIR="${REPO_ROOT}/artifacts/phase2/snapshot-pre-live-activation-${TIMESTAMP}"
...
abort_with_rollback() {
    ...
    echo "  sudo bash ${SCRIPT_DIR}/rollback-snapshot-pre-slice.sh ${ARTIFACTS_DIR}" >&2
    echo "" >&2
...
# snapshot_pre slice activation result
...
# 2.3 Verify source contains snapshot_pre in ENABLED_ACTIONS
if ! grep -q '"snapshot_pre"' "$REPO_PLUGIN_SRC"; then
    log_fail "Source plugin does not contain snapshot_pre in ENABLED_ACTIONS"
    exit 1
fi
log_ok "Source plugin contains snapshot_pre in ENABLED_ACTIONS"
...
================================================================
  snapshot_pre Slice Activation — Result Summary
================================================================
...
2. In a FRESH Claude Code session, run agent-facing E2E tests:
   - Positive: snapshot_pre with valid label+reason
   - Negative: invalid label, missing reason, non-object inputs
   - Regression: gateway_health still works
3. Do NOT mark as live verified until E2E tests pass
4. If issues found, rollback with:
   sudo bash ${SCRIPT_DIR}/rollback-snapshot-pre-slice.sh ${ARTIFACTS_DIR}
================================================================
SUMMARYEOF
...
echo ""
echo "========================================================"
echo "  ACTIVATION COMPLETE — snapshot_pre slice deployed"
echo "========================================================"
echo ""
...
echo "    sudo bash ${SCRIPT_DIR}/rollback-snapshot-pre-slice.sh ${ARTIFACTS_DIR}"
echo ""
