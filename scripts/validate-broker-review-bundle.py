#!/usr/bin/env python3
"""Validate repo-side broker review bundle artifacts."""
...
SCHEMA_PATH = REPO_ROOT / "schemas" / "broker-review-bundle.schema.json"


def usage() -> int:
    print("Usage: scripts/validate-broker-review-bundle.py [--expect-valid|--expect-invalid] FILE [FILE ...]")
    return 2
...
def validate_validation_status(value: object, errors: list[str], where: str) -> None:
    ...
    reject_extra_keys(
        value,
        {"normalized_request", "intake_report", "routing_decision", "submission_envelope"},
        errors,
        where,
    )
    for key in ("normalized_request", "intake_report", "routing_decision", "submission_envelope"):
        ensure(value.get(key) in VALIDATION_RESULTS, errors, f"{where}.{key} must be passed or failed")
...
def validate_document(path: Path) -> list[str]:
    errors = validate_schema_file(
        SCHEMA_PATH,
        expected_title="BrokerReviewBundle",
        expected_const="openclaw.main.broker-review-bundle.v1alpha1",
    )
    if errors:
        return errors
    ...
    ensure(data.get("schema_version") == "openclaw.main.broker-review-bundle.v1alpha1", errors, f"{path}: schema_version mismatch")
    ensure(data.get("bundle_type") == "BROKER_REVIEW_BUNDLE", errors, f"{path}: bundle_type must be BROKER_REVIEW_BUNDLE")
    ensure(data.get("review_target") == "broker", errors, f"{path}: review_target must be broker")
    ensure(data.get("review_status") == "pending_broker_review", errors, f"{path}: review_status must be pending_broker_review")
    ensure(data.get("submission_envelope_type") == "BROKER_SUBMISSION_CANDIDATE", errors, f"{path}: submission_envelope_type must be BROKER_SUBMISSION_CANDIDATE")
    ensure(data.get("decision") == "BROKER_SUBMISSION_CANDIDATE", errors, f"{path}: decision must be BROKER_SUBMISSION_CANDIDATE")
    ensure(data.get("submission_target") == "broker_submission_envelope", errors, f"{path}: submission_target must be broker_submission_envelope")
    ensure(data.get("dispatch_scope") == "readonly", errors, f"{path}: dispatch_scope must be readonly")
    ensure(data.get("dispatch_status") == "broker_ready", errors, f"{path}: dispatch_status must be broker_ready")
    ensure(data.get("requires_operator_approval") is False, errors, f"{path}: requires_operator_approval must be false")
    ensure(is_non_empty_string(data.get("rationale")), errors, f"{path}: rationale must be non-empty")
    ensure(data.get("approval_required_reason") is None, errors, f"{path}: approval_required_reason must be null")
    ensure(data.get("refusal_reason") is None, errors, f"{path}: refusal_reason must be null")
    ...
    for key in ("bundle_id", "submission_envelope_id", "routing_decision_id", "intake_id", "task_id", "run_id", "source_request_id"):
    ...
    if ensure_type(source_artifacts, dict, errors, f"{path}: source_artifacts must be an object"):
        reject_extra_keys(
            source_artifacts,
            {"normalized_request_path", "intake_report_path", "routing_decision_path", "submission_envelope_path"},
            errors,
            f"{path}.source_artifacts",
        )
        for key in ("normalized_request_path", "intake_report_path", "routing_decision_path", "submission_envelope_path"):
