#!/usr/bin/env python3
"""Validate repo-side operator review bundle artifacts."""
...
SCHEMA_PATH = REPO_ROOT / "schemas" / "operator-review-bundle.schema.json"


def usage() -> int:
    print("Usage: scripts/validate-operator-review-bundle.py [--expect-valid|--expect-invalid] FILE [FILE ...]")
    return 2
...
def validate_document(path: Path) -> list[str]:
    errors = validate_schema_file(
        SCHEMA_PATH,
        expected_title="OperatorReviewBundle",
        expected_const="openclaw.main.operator-review-bundle.v1alpha1",
    )
    if errors:
        return errors
    ...
    ensure(data.get("schema_version") == "openclaw.main.operator-review-bundle.v1alpha1", errors, f"{path}: schema_version mismatch")
    ensure(data.get("bundle_type") == "OPERATOR_REVIEW_BUNDLE", errors, f"{path}: bundle_type must be OPERATOR_REVIEW_BUNDLE")
    ensure(data.get("review_target") == "operator", errors, f"{path}: review_target must be operator")
    ensure(data.get("review_status") == "pending_operator_review", errors, f"{path}: review_status must be pending_operator_review")
    ensure(data.get("submission_envelope_type") == "REQUIRES_OPERATOR_APPROVAL", errors, f"{path}: submission_envelope_type must be REQUIRES_OPERATOR_APPROVAL")
    ensure(data.get("decision") == "REQUIRES_OPERATOR_APPROVAL", errors, f"{path}: decision must be REQUIRES_OPERATOR_APPROVAL")
    ensure(data.get("submission_target") == "operator_approval_envelope", errors, f"{path}: submission_target must be operator_approval_envelope")
    ...
    ensure(is_non_empty_string(data.get("approval_required_reason")), errors, f"{path}: approval_required_reason must be non-empty")
