#!/usr/bin/env python3
"""Validate repo-side broker submission envelope artifacts."""
...
SCHEMA_PATH = REPO_ROOT / "schemas" / "broker-submission-envelope.schema.json"


def usage() -> int:
    print("Usage: scripts/validate-broker-submission-envelope.py [--expect-valid|--expect-invalid] FILE [FILE ...]")
    return 2
...
def validate_document(path: Path) -> list[str]:
    errors = validate_schema_file(
        SCHEMA_PATH,
        expected_title="BrokerSubmissionEnvelope",
        expected_const="openclaw.main.broker-submission-envelope.v1alpha1",
    )
    if errors:
        return errors
    ...
    ensure(data.get("schema_version") == "openclaw.main.broker-submission-envelope.v1alpha1", errors, f"{path}: schema_version mismatch")
    ensure(data.get("envelope_mode") == "repo_side_dry_run", errors, f"{path}: envelope_mode must be repo_side_dry_run")
    ensure(data.get("envelope_type") == "BROKER_SUBMISSION_CANDIDATE", errors, f"{path}: envelope_type must be BROKER_SUBMISSION_CANDIDATE")
