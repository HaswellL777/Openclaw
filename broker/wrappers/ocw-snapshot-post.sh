#!/usr/bin/env bash
# ocw-snapshot-post.sh — Wrapper for snapshot_post action
...
# In live mode: requires root, creates post-change btrfs snapshot
#
...
if [ "${BROKER_DRY_RUN:-true}" = "true" ]; then
  ...
  broker_emit_result \
    ...
    "[STUB] Post-snapshot — dry-run, no live execution" \
