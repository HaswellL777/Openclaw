...
- 当前是 **Phase 2 agent-facing 切片推进中**（两项只读 + 一项写操作已验收，其余仍未开放）：
  ...
  - **其余 5 个 action 仍需逐项 agent-facing 开放与验收**
