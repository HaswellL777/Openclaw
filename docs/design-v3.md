...
1. **当前真实落地阶段是 Phase 1（Phase 1A + Phase 1B 均已完成）+ Phase 2 broker deployment 已完成，Phase 2 plugin activation 已完成，agent-facing host_ops 逐项切片推进中（gateway_health + validate_openclaw_json_candidate + deploy_openclaw_json_candidate + snapshot_pre + snapshot_post + rollback_prepare 已 live verified）。**
2. **主控制面仍在宿主机，不容器化。**
...
13. **Claude Code 容器内执行链仍属于后续阶段目标；agent-facing `host_ops` 已完成六个切片（`gateway_health` + `validate_openclaw_json_candidate` + `deploy_openclaw_json_candidate` + `snapshot_pre` + `snapshot_post` + `rollback_prepare`，均 live E2E verified，最新 2026-03-16），其余 2 个 action 仍需逐项验证开放；除非特别注明”已验证”，否则不得写成当前事实。**
...
> **部署状态（2026-03-16）**：broker daemon 已部署并运行（`openclaw-broker.service`，active + enabled）；8 个 wrapper 已安装为 production 版本；host-ops-tool plugin 已注册进 `openclaw.json`。plugin activation 已完成，registerTool 版 plugin 已部署到 live。agent-facing 已逐项开放六个切片：`gateway_health`（只读）、`validate_openclaw_json_candidate`（只读）、`deploy_openclaw_json_candidate`（写操作，Route C）、`snapshot_pre`（写操作，创建只读快照）、`snapshot_post`（写操作，创建只读快照）、`rollback_prepare`（纯只读，验证 snapshot 存在性并返回 prepare-only metadata，不返回 rollback_steps，不执行实际 rollback），均已 live E2E verified。deploy 后的 restart/snapshot 仍由 operator-mediated checklist 承担。其余 2 个 action（`gateway_restart`, `vault_sync`）仍需逐项开放。
