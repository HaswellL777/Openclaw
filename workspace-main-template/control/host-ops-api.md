...
`gateway_health` 和 `validate_openclaw_json_candidate` 两个 action 均已通过 agent-facing E2E 验证（live verified），可由 agent 直接调用。两者均为只读操作。`deploy_openclaw_json_candidate`（写操作）已在 repo-side 开放（Route C），待 live 实施后可由 agent 调用，但 deploy 后的 restart / snapshot 仍由 operator-mediated checklist 承担（见 `docs/checklists/deploy-candidate-route-c-checklist.md`）。其余 5 个 action（含写操作）尚未逐项 agent-facing 验收，仍使用 Phase 1 workaround（Section "Phase 1 workaround"）。
