...
const SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "状态目录（openclaw:openclaw 700）", content: "/var/lib/openclaw/.openclaw/" },
      { label: "main workspace", content: "/var/lib/openclaw/.openclaw/workspace-main" },
      { label: "auditor workspace", content: "/var/lib/openclaw/workspace-auditor" },
      { label: "research-coordinator workspace", content: "/var/lib/openclaw/workspace-research-coordinator" },
      { label: "task-runner 工作目录", content: "/var/lib/openclaw/task-workspaces/（按任务动态创建）" },
      ...
      { label: "Broker", content: "/var/lib/openclaw/broker/" },
      { label: "日志", content: "/var/log/openclaw/" },
      { label: "快照", content: "/.snapshots/" },
      { label: "注意：没有 agents/ 目录", content: "workspace 路径由 config 中 agents.list[].workspace 字段指定，不是固定的 agents/<id>/ 结构。" },
    ],
  },
  {
    title: "快照与备份",
  ...
  },
  {
    title: "Agent 工作区发布",
    items: [
      { label: "发布 main 工作区（交互式确认）", content: "sudo bash scripts/publish-workspace-main.sh --apply --allow-live-target /var/lib/openclaw/.openclaw/workspace-main" },
      { label: "手动 rsync 其他 agent", content: "sudo rsync -av workspace-<agent>-template/ /var/lib/openclaw/workspace-<agent>/" },
      { label: "发布后修复权限", content: "sudo chown -R openclaw:openclaw /var/lib/openclaw/workspace-<agent>" },
      { label: "警告：chown -R 会在 knowledge/ 上失败", content: "knowledge/ 是只读绑定挂载，不能递归 chown。需要逐个路径指定。" },
      { label: "workspace 路径查看", content: "workspace 路径由 config 中各 agent 配置决定，可在 GUI Settings 页查看。不是固定的 agents/<id> 结构。" },
    ],
  },
  {
    title: "故障排查",
